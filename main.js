...
function writeRaOverride(scope, refId) {
    ...
    if (data.monitor      != null && data.monitor      !== '') lines.push(`video_monitor_index = "${data.monitor}"`);
    if (data.fullscreen   != null && data.fullscreen   !== '') lines.push(`video_fullscreen = "${data.fullscreen}"`);
...
ipcMain.handle('launch-game-ex', (_, gameId, opts = {}) => {
    ...
    if (/retroarch/i.test(cmd)) {
        const files = raOverrideFiles(game);
        if (opts.fresh) {
